...
public class GeneralAdvancedTechniques : MonoBehaviour {
	...
	void Start () {
		...
		// Move to path of childTransforms that are moving themselves
		...
		for(int i = 0; i < avatarSpeed.Length; i++)
			LeanTween.moveLocalZ( avatarSpeed[i], (i+1)*5f, 1f).setSpeed(6f).setEase(LeanTweenType.easeInOutExpo).setLoopPingPong(); // any timeUI you set the speed it overrides the timeUI value
